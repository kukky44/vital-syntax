class Squares extends PatternItem {
  ...
  draw(g = window) {
    ...
    //horizontal 2
    if(this.num === 1) {
      g.rect(-this.gap, 0, this.size);
      g.rect(this.gap, 0, this.size);
    }

    //vertical 2
    if(this.num === 2) {
      g.rect(0, -this.gap, this.size);
      g.rect(0, this.gap, this.size);
    }
    ...
    if(this.num === 3) {
      ...
      g.rotate(QUARTER_PI);
      g.rect(-scaledGap, -scaledGap, scaled);
      g.rect(scaledGap, scaledGap, scaled);
    }

    //vertical 3
    if(this.num === 4) {
      g.rect(0, -this.gap*0.9, this.size);
      g.rect(-this.gap, this.gap, this.size);
      g.rect(this.gap, this.gap, this.size);
    }
    ...
    if(this.num === 6) {
      ...
      g.rect(0, wrap, scaled);
      g.rect(-wrap, -wrap, scaled);
      g.rect(wrap, 0, scaled);
    }
